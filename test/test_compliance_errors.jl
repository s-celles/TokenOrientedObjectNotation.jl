...
@testset "Compliance: Error Conditions (§14)" begin
    @testset "Array Count Mismatch Errors" begin
        ...
        @test_throws Exception TOON.decode("[3]: 1,2")  # Too few
        @test_throws Exception TOON.decode("[3]: 1,2,3,4")  # Too many
        @test_throws Exception TOON.decode("[5]: 1,2,3")  # Declared 5, got 3
        ...
        @test_throws Exception TOON.decode("[1]{a,b}:\n  1,2\n  3,4")  # Too many rows
        
        # Non-strict mode accepts mismatches
        opts = TOON.DecodeOptions(strict=false)
        @test_nowarn TOON.decode("[5]: 1,2,3", options=opts)
        @test_nowarn TOON.decode("[2]:\n  - a\n  - b\n  - c", options=opts)
    end
    
    @testset "Row Width Mismatch Errors" begin
        # Too few values in row
        @test_throws Exception TOON.decode("[2]{a,b,c}:\n  1,2,3\n  4,5")
    ...
    @testset "Invalid Escape Sequence Errors" begin
        ...
        @test_throws ArgumentError TOON.decode("text: \"test\\x41\"")  # \x not valid
        @test_throws ArgumentError TOON.decode("text: \"test\\u0041\"")  # \u not valid
        @test_throws ArgumentError TOON.decode("text: \"test\\a\"")  # \a not valid
        @test_throws ArgumentError TOON.decode("text: \"test\\b\"")  # \b not valid
        @test_throws ArgumentError TOON.decode("text: \"test\\f\"")  # \f not valid
        @test_throws ArgumentError TOON.decode("text: \"test\\v\"")  # \v not valid
        @test_throws ArgumentError TOON.decode("text: \"test\\0\"")  # \0 not valid
        @test_throws ArgumentError TOON.decode("text: \"test\\z\"")  # \z not valid
        
        # Only five escapes are valid: \\, \", \n, \r, \t
        @test_nowarn TOON.decode("text: \"test\\\\value\"")
        @test_nowarn TOON.decode("text: \"test\\\"value\"")
        @test_nowarn TOON.decode("text: \"test\\nvalue\"")
        @test_nowarn TOON.decode("text: \"test\\rvalue\"")
        @test_nowarn TOON.decode("text: \"test\\tvalue\"")
    end
    ...
    @testset "Unterminated String Errors" begin
        ...
        @test_throws Exception TOON.decode("text: \"missing end quote")
        @test_throws Exception TOON.decode("[2]: \"a\",\"b")
    ...
    @testset "Invalid Header Format Errors" begin
        ...
        # @test_throws Exception TOON.decode("[abc]: 1,2,3")
        # @test_throws Exception TOON.decode("[N]: 1,2,3")
        
        # Negative array length - may be parsed as key
        # @test_throws Exception TOON.decode("[-1]: 1,2,3")
        ...
        # @test_throws Exception TOON.decode("[3;]: 1;2;3")  # Semicolon not valid
    end
    ...
    @testset "Edge Case Errors" begin
        ...
        @test_nowarn TOON.encode(deep)
        @test_nowarn TOON.decode(TOON.encode(deep))
        
        # Very long strings (should work)
        long_str = "x" ^ 10000
        @test_nowarn TOON.encode(long_str)
        @test_nowarn TOON.decode(TOON.encode(long_str))
        
        # Very large arrays (should work)
        large_arr = collect(1:10000)
        @test_nowarn TOON.encode(large_arr)
