...
@testset "Indentation and Whitespace" begin
    @testset "Requirement 9.1: Consistent spaces per level" begin
        ...
        result = TOON.encode(obj, options=options)
        lines = split(result, '\n')
        @test lines[1] == "a:"
        @test lines[2] == "    b:"
        @test lines[3] == "        c: 1"
    ...
    @testset "Requirement 9.5: No trailing spaces at end of lines" begin
        ...
        result = TOON.encode(obj_with_empty)
        lines = split(result, '\n')
        for line in lines
            @test !endswith(line, " ")
        end
    ...
    @testset "Requirement 9.6: No trailing newline at end of document" begin
        # Simple value
        @test !endswith(TOON.encode(42), '\n')
        @test !endswith(TOON.encode("hello"), '\n')
        ...
        result = TOON.encode(complex)
        @test !endswith(result, '\n')
        
        # Empty structures
        @test !endswith(TOON.encode(Dict()), '\n')
        @test !endswith(TOON.encode([]), '\n')
    end

    @testset "Requirement 9.7: Strict mode validates indentation as exact multiple" begin
        # Valid indentation (multiples of 2)
        valid_input = "a:\n  b: 1"
        @test_nowarn TOON.decode(valid_input, options=TOON.DecodeOptions(indent=2, strict=true))
        
        # Valid indentation (multiples of 4)
        valid_input = "a:\n    b: 1"
        @test_nowarn TOON.decode(valid_input, options=TOON.DecodeOptions(indent=4, strict=true))
        
        # Invalid indentation (3 spaces with indent=2)
        invalid_input = "a:\n   b: 1"
        @test_throws Exception TOON.decode(invalid_input, options=TOON.DecodeOptions(indent=2, strict=true))
        ...
        invalid_input = "a:\n  b: 1"
        @test_throws Exception TOON.decode(invalid_input, options=TOON.DecodeOptions(indent=4, strict=true))
        
        # Non-strict mode allows non-multiples
        lenient_input = "a:\n   b: 1"
        result = TOON.decode(lenient_input, options=TOON.DecodeOptions(indent=2, strict=false))
        @test haskey(result, "a")
        
        # Deep nesting with valid indentation
        deep_valid = "a:\n  b:\n    c:\n      d: 1"
        @test_nowarn TOON.decode(deep_valid, options=TOON.DecodeOptions(indent=2, strict=true))
        
        # Deep nesting with invalid indentation at one level
        deep_invalid = "a:\n  b:\n   c:\n      d: 1"
        @test_throws Exception TOON.decode(deep_invalid, options=TOON.DecodeOptions(indent=2, strict=true))
    end
    
    @testset "Requirement 9.8: Strict mode rejects tabs in indentation" begin
        # Tab in indentation
        tab_input = "\tvalue: 1"
        @test_throws Exception TOON.decode(tab_input, options=TOON.DecodeOptions(strict=true))
        
        # Tab mixed with spaces
        mixed_input = "  \tvalue: 1"
        @test_throws Exception TOON.decode(mixed_input, options=TOON.DecodeOptions(strict=true))
        
        # Tab at deeper level
        nested_tab = "a:\n\tb: 1"
        @test_throws Exception TOON.decode(nested_tab, options=TOON.DecodeOptions(strict=true))
        ...
        @test_nowarn TOON.decode(array_with_tab, options=TOON.DecodeOptions(strict=true))
        
        # Tab in string value is OK (not indentation)
        string_with_tab = "text: \"hello\tworld\""
        @test_nowarn TOON.decode(string_with_tab, options=TOON.DecodeOptions(strict=true))
        
        # Non-strict mode is more lenient - tabs are allowed but treated as single space
        # This is implementation-dependent behavior
        result = TOON.decode("\tvalue: 1", options=TOON.DecodeOptions(strict=false))
    ...
    @testset "Edge Cases: Indentation and Whitespace" begin
        ...
        result = TOON.decode(input_with_blanks, options=TOON.DecodeOptions(strict=false))
        @test haskey(result, "a")
        @test haskey(result, "b")
        
        # Very deep nesting
        deep = Dict("a" => Dict("b" => Dict("c" => Dict("d" => Dict("e" => 1)))))
        result = TOON.encode(deep)
        lines = split(result, '\n')
        @test lines[1] == "a:"
        @test lines[2] == "  b:"
        @test lines[3] == "    c:"
        ...
        result = TOON.encode(arr_with_obj)
