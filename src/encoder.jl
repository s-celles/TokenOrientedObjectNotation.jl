...
"""
function collect_all_folded_paths(key::String, value::JsonValue, prefix::String="")::Vector{String}
    paths = String[]
    ...
    elseif is_json_object(value)
        ...
        if should_fold && !is_empty_object(value)
            ...
            if length(value) == 1
                ...
                child_can_fold = is_safe_identifier(child_key) && is_valid_unquoted_key(child_key)

                # In safe mode at root level, check for collision with sibling keys
                has_collision = false
                if options.keyFolding == "safe" && depth == 0 && parent_obj !== nothing && isempty(prefix)
                ...
                if !has_collision && (child_can_fold || options.keyFolding != "safe")
                    ...
                    for (child_key, child_value) in value
                        encode_key_value_pair(child_key, child_value, writer, depth, options, prefix=full_key, parent_obj=nothing)
                    end
                else
